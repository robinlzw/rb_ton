...
if [ ! -z "$PUBLIC_IP" ]; then
    ...
    exec /ton/build/validator-engine/validator-engine -C /var/ton-work/db/ton-global.config --db /var/ton-work/db --ip "$PUBLIC_IP:$CONSOLE_PORT"
else
...
# else 
    ...
    read -r SERVER_ID1 SERVER_ID2 <<< $(exec /ton/build/utils/generate-random-id -m keys -n server)
...
# else
    read -r CLIENT_ID1 CLIENT_ID2 <<< $(exec /ton/build/utils/generate-random-id -m keys -n client)
...
if [ -z "$LITESERVER" ]; then
    ...
    if [ -f "./liteserver" ]; then
        ...
        read -r LITESERVER_ID1 LITESERVER_ID2 <<< $(exec /ton/build/utils/generate-random-id -m keys -n liteserver)
...
mkdir -p /var/ton-work/log/
exec /ton/build/validator-engine/validator-engine -c /var/ton-work/db/config.json -C /var/ton-work/db/ton-global.config --db /var/ton-work/db -l /var/ton-work/log/log