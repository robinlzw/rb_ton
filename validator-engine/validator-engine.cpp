...
/*
这段代码是一个C++程序，属于TON区块链的一部分。它定义了一个名为`ValidatorEngine`的类，用于运行TON区块链的验证者节点。以下是代码的主要功能和组成部分的概述：

1. **文件包含**：
   - 包含了一系列TON区块链相关的头文件，如`adnl.h`、`ton_api.h`、`rldp.h`等，这些文件提供了网络通信、数据结构、验证者管理等功能。
   - 还包括了一些通用的C++库，如`td/actor/actor.h`、`td/utils/filesystem.h`等，用于处理文件系统、日志记录等。

2. **ValidatorEnginePermissions 枚举**：
   - 定义了验证者引擎的权限级别，如`vep_default`、`vep_modify`、`vep_unsafe`。

3. **Config 结构**：
   - 定义了一个配置结构体，包含了网络地址、验证者信息、控制接口等配置项。
   - 提供了一些方法来添加或删除网络地址、ADNL地址、DHT节点、验证者密钥等。

4. **ValidatorEngine 类**：
   - 继承自`td::actor::Actor`，是一个actor类，用于处理异步任务。
   - 包含了TON区块链网络所需的各种组件的成员变量，如密钥环(`Keyring`)、ADNL网络管理器(`AdnlNetworkManager`)、DHT节点(`Dht`)、
   覆盖网络管理器(`Overlays`)、验证者管理接口(`ValidatorManagerInterface`)等。
   - 提供了一系列方法来管理网络地址、ADNL地址、DHT节点、验证者密钥等，如`add_addr`、`add_adnl`、`add_dht`等。
   - 提供了启动网络、DHT、覆盖网络、验证者、全节点等组件的方法，如`start_adnl`、`start_dht`、`start_overlays`、`start_validator`、`start_full_node`等。
   - 提供了控制接口和控制查询的处理方法，允许用户通过控制接口执行各种操作，如获取时间、导入私钥、导出公钥、生成密钥对、添加ADNL ID、添加DHT ID、
   添加验证者永久密钥、添加验证者临时密钥、添加验证者ADNL地址、更改全节点ADNL地址、添加Liteserver、添加控制接口、删除ADNL ID、删除DHT ID、
   删除验证者永久密钥、删除验证者临时密钥、删除验证者ADNL地址等。

5. **main 函数**：
   - 是程序的入口点，初始化并运行`ValidatorEngine`。
   - 使用`td::OptionParser`解析命令行参数，设置日志记录、数据库根目录、IP地址等。
   - 创建并启动`ValidatorEngine`actor，处理各种配置和启动逻辑。

6. **辅助函数**：
   - `create_control_query_error`：创建控制查询错误消息。
   - `run_control_query`：处理不同类型的控制查询。
   - `process_control_query`：处理接收到的控制查询。

7. **信号处理**：
   - 使用`td::set_runtime_signal_handler`设置信号处理程序，用于处理统计信息、调度器状态、日志旋转等。

8. **内存管理**：
   - 使用`memprof`库进行内存管理的调试。

9. **调度器**：
   - 使用`td::actor::Scheduler`进行线程调度，处理异步任务。

整体来看，这个程序是一个完整的TON区块链验证者节点的实现，包含了网络通信、密钥管理、配置管理、控制接口处理等功能。

*/

// ===========================================================================================
/*
在提供的代码中，`ValidatorEnginePermissions` 枚举定义了不同级别的权限，这些权限用于控制对验证者引擎的不同操作。以下是权限级别及其在代码中的体现和实现方式：

```cpp
enum ValidatorEnginePermissions : td::uint32 {
    vep_default = 1,   // 默认权限
    vep_modify = 2,     // 修改权限
    vep_unsafe = 4      // 不安全权限
};
...
1. **vep_default (默认权限)**：
   - 这个权限级别允许用户执行一些基本的操作，比如获取时间、导出公钥等。
   - 在 `run_control_query` 函数中，许多查询处理函数检查权限时会使用这个级别。例如：
     ```cpp
     if (!(perm & ValidatorEnginePermissions::vep_default)) {
         promise.set_value(create_control_query_error(td::Status::Error(ton::ErrorCode::error, "not authorized")));
         return;
...
2. **vep_modify (修改权限)**：
   - 这个权限级别允许用户执行一些修改系统状态的操作，比如添加或删除ADNL地址、DHT节点、验证者密钥等。
   - 在 `run_control_query` 函数中，涉及修改操作的查询处理函数会检查这个权限。例如：
     ```cpp
     if (!(perm & ValidatorEnginePermissions::vep_modify)) {
         promise.set_value(create_control_query_error(td::Status::Error(ton::ErrorCode::error, "not authorized")));
         return;
...
3. **vep_unsafe (不安全权限)**：
   - 这个权限级别允许用户执行一些潜在危险或不推荐的操作，通常这些操作可能会影响系统的稳定性或安全性。
   - 在 `run_control_query` 函数中，涉及不安全操作的查询处理函数会检查这个权限。例如：
     ```cpp
     if (!(perm & ValidatorEnginePermissions::vep_unsafe)) {
         promise.set_value(create_control_query_error(td::Status::Error(ton::ErrorCode::error, "not authorized")));
         return;
...
- **权限检查**：在处理控制查询时，首先检查发起查询的用户是否具有足够的权限。这是通过检查用户权限与所需权限的位运算结果来实现的。
- **错误处理**：如果用户没有足够的权限，返回一个错误响应，通常是通过 `create_control_query_error` 函数创建的。
- **操作执行**：如果用户具有足够的权限，继续执行相应的操作，比如修改配置、执行签名等。

这种权限控制机制确保了验证者引擎的安全性和灵活性，允许系统管理员根据需要对不同的操作进行细粒度的权限管理。

*/

// ===========================================================================================
/*
`ValidatorEngine` 类是 TON 区块链中的一个核心组件，它负责协调和管理网络中的各种活动，包括网络通信、数据存储、验证者管理等。
以下是 `ValidatorEngine` 类如何通过其成员变量关联并实现业务的详细说明：

1. **密钥环 (`Keyring`)**：
   - `keyring_` 成员变量负责管理密钥对，包括生成、存储和检索公钥和私钥。
   - 在区块链中，密钥对用于身份验证和交易签名。`ValidatorEngine` 使用密钥环来访问和管理系统密钥。

2. **ADNL 网络管理器 (`AdnlNetworkManager`)**：
   - `adnl_network_manager_` 负责管理 ADNL（自动数据网络层）网络，这是一种用于 TON 网络的点对点通信协议。
   - 它处理网络连接的建立、维护和关闭，以及网络发现和路由。

3. **ADNL 实例 (`Adnl`)**：
   - `adnl_` 是 ADNL 协议的具体实现，用于处理实际的网络通信。
   - 它与 `AdnlNetworkManager` 配合工作，负责发送和接收消息。

4. **DHT 节点 (`Dht`)**：
   - `dht_nodes_` 是一个存储 DHT（分布式哈希表）节点的映射，每个节点负责处理网络中的特定数据分片。
   - DHT 用于实现去中心化的数据存储和检索，`ValidatorEngine` 通过 DHT 节点参与网络的数据存储和检索。

5. **覆盖网络管理器 (`Overlays`)**：
   - `overlay_manager_` 负责管理覆盖网络，这是 TON 网络中的一个关键组件，用于处理消息的传播和同步。
   - 它管理网络中的不同层次的覆盖网络，确保消息能够在网络中正确传播。

6. **验证者管理接口 (`ValidatorManagerInterface`)**：
   - `validator_manager_` 是验证者管理的核心组件，负责协调验证者的活动，包括区块的验证和选举。
   - 它处理验证者的注册、选举和状态管理，确保网络的安全性和一致性。

7. **全节点客户端 (`AdnlExtClient`)**：
   - `full_node_client_` 是一个 ADNL 客户端，用于与全节点通信。
   - 全节点是 TON 网络中的完整节点，拥有整个区块链的数据，`ValidatorEngine` 通过全节点客户端与之交互，获取数据和同步状态。

8. **全节点 (`FullNode`)**：
   - `full_node_` 是 TON 区块链的全节点实现，负责存储和管理区块链数据。
   - 它处理区块的验证、存储和广播，是网络中的关键数据源。

9. **全节点主节点 (`FullNodeMaster`)**：
   - `full_node_masters_` 是一个存储全节点主节点的映射，每个主节点负责处理特定工作链的全节点任务。
   - 主节点与全节点配合工作，提供工作链特定的数据和状态管理。

10. **控制外部服务器 (`AdnlExtServer`)**：
    - `control_ext_server_` 是一个 ADNL 服务器，用于处理来自外部的控制命令和查询。
    - 它允许用户通过 ADNL 网络发送控制命令，如获取状态、执行交易等。

在 `ValidatorEngine` 的实现中，这些组件通过以下方式协同工作：

- **网络通信**：通过 ADNL 网络管理器和 ADNL 实例，`ValidatorEngine` 能够与其他网络节点建立通信，发送和接收消息。
- **数据存储**：通过 DHT 节点和全节点，`ValidatorEngine` 能够存储和检索区块链数据，保持数据的一致性和完整性。
- **验证者管理**：通过验证者管理接口，`ValidatorEngine` 能够管理验证者的活动，确保网络的安全性和稳定性。
- **控制接口**：通过控制外部服务器，`ValidatorEngine` 能够响应外部控制命令，提供灵活的接口用于网络管理和监控。

通过这种方式，`ValidatorEngine` 成为 TON 区块链网络中的核心枢纽，协调和管理网络中的各种活动。
...
/*
当然，让我们详细了解一下 ADNL（自动数据网络层）网络管理器和 DHT（分布式哈希表）节点在 TON 区块链网络中的作用和区别。

### ADNL 网络管理器 (`AdnlNetworkManager`)

**ADNL** 是 TON 区块链中用于节点间通信的底层网络协议。ADNL 网络管理器主要负责以下功能：

1. **网络发现**：管理器负责发现网络上的其他节点，这通常通过静态配置、动态发现或其他机制完成。

2. **连接管理**：建立和维护与网络中其他节点的连接。这包括处理连接的建立、断开和重连。

3. **路由**：在去中心化的网络中，ADNL 网络管理器需要能够将消息路由到正确的节点，即使目标节点不直接连接到发送者。

4. **消息传输**：负责消息的发送和接收。ADNL 协议支持可靠和不可靠的消息传输，允许节点间交换各种类型的数据。

5. **安全性**：ADNL 网络管理器还负责网络通信的安全性，包括加密和认证机制，确保数据传输的安全。

6. **代理支持**：在某些情况下，网络管理器可能需要通过代理服务器与外部网络通信，这在 NAT 环境或防火墙限制的环境中很常见。

### DHT 节点 (`Dht`)

**DHT** 是一种分布式存储和检索系统，它允许网络中的节点存储和检索数据。在 TON 区块链中，DHT 节点的作用包括：

1. **数据存储**：每个 DHT 节点负责存储网络中的部分数据。这些数据可以是区块链的一部分、状态信息或其他元数据。

2. **数据检索**：当一个节点需要获取数据时，它可以查询 DHT 网络，DHT 节点会返回请求的数据。

3. **数据一致性**：DHT 系统通常具有数据复制和一致性机制，确保网络中的数据在多个节点间保持一致。

4. **去中心化**：DHT 网络是去中心化的，没有单一的控制点，这使得系统更加健壮和抗审查。

5. **可扩展性**：DHT 系统通常具有良好的可扩展性，随着网络节点的增加，系统的存储和检索能力也随之增加。

### 区别

- **作用范围**：ADNL 网络管理器主要负责节点间的通信和网络维护，而 DHT 节点主要负责数据的存储和检索。

- **数据类型**：ADNL 网络管理器处理的是网络通信的消息，而 DHT 节点处理的是区块链数据和状态信息。

- **去中心化程度**：虽然两者都是去中心化的，但 DHT 网络通常更加去中心化，因为它依赖于网络中的每个节点来存储和检索数据。

- **安全性**：ADNL 网络管理器更关注通信的安全性，如加密和认证，而 DHT 节点更关注数据的一致性和完整性。

- **性能要求**：ADNL 网络管理器需要快速响应网络变化和通信需求，而 DHT 节点则需要高效地存储和检索数据，可能涉及到更复杂的数据复制和一致性算法。

在 TON 区块链中，ADNL 网络管理器和 DHT 节点共同工作，确保网络的通信效率和数据的可靠性。ADNL 网络管理器负责将数据传输到正确的节点，而 DHT 节点则负责存储和检索这些数据。
