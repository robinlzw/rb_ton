...
/*
这个函数 `TestNode::run_smc_method` 是 TON 区块链中的一个函数，它负责执行智能合约方法。以下是函数的详细解释：

### 参数列表
- `mode`: 执行模式，可能影响虚拟化和结果验证。
- `ref_blk`: 参考区块 ID，用于验证状态。
- `blk`: 当前区块 ID。
- `shard_blk`: 分片区块 ID。
- `shard_proof`: 分片证明数据。
- `proof`: 账户状态证明数据。
...
- `remote_c7`: 远程环境变量 C7。
- `remote_libs`: 远程库。
- `remote_result`: 远程结果数据。
- `remote_exit_code`: 远程退出代码。
- `promise`: 返回结果的承诺对象。

### 函数逻辑
1. **日志记录**：记录接收到的账户状态和相关区块信息。
2. **账户状态验证**：使用 `account_state.validate` 方法验证账户状态。
3. **状态检查**：检查账户状态是否为空，如果为空，则无法执行方法。
4. **反序列化**：尝试从 `state` 中反序列化账户状态。
5. **状态初始化**：检查账户是否初始化，如果没有初始化或被冻结，则无法执行方法。
6. **方法执行**：
   - 设置虚拟机状态和限制。
   - 准备方法调用参数，包括方法 ID 和参数。
   - 创建虚拟机实例并运行方法。
7. **错误处理**：如果虚拟机运行过程中出现错误，记录错误并设置承诺错误。
8. **结果处理**：
   - 如果模式允许，使用远程结果而不是本地结果。
   - 如果本地或远程虚拟机退出代码非零，记录错误并设置承诺错误。
   - 如果一切正常，将虚拟机的栈内容作为结果设置到承诺中。

### 代码细节
- **日志记录**：使用 `LOG` 宏记录函数执行的各个阶段，有助于调试和监控。
- **验证和错误处理**：在执行方法之前，进行多次验证和错误处理，确保状态有效且可执行。
- **虚拟机运行**：使用 TON 虚拟机执行智能合约方法，处理方法调用和结果。
- **结果承诺**：根据执行模式和结果，将结果或错误设置到 `promise` 中，以便异步返回。

### 总结
`TestNode::run_smc_method` 函数是 TON 区块链中智能合约交互的核心部分，负责验证账户状态、执行智能合约方法，并处理结果。它通过详细的日志记录和严格的错误处理，确保智能合约的安全可靠执行。
