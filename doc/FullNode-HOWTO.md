...
您可以将全节点设置为 Lite 服务器，以便从同一台或远程主机使用 Lite 客户端连接到它。例如，在连接到全节点的 Lite 客户端中发送命令 `last` 将显示全节点已知的最新主链区块的标识符，以便您检查区块下载的进度。您还可以检查所有智能合约的状态，发送外部消息（例如钱包查询）等，正如 Lite 客户端 HOWTO 中所解释的那样。
